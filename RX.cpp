...
// Here we check the outputs from the IR sensors to determine if
// the drone is approaching an obstacle
void avoid()
{
  // If either of the front IR sensors detect an object we set front to true
  if(!digitalRead(IR_FRONT_L) || !digitalRead(IR_FRONT_R))
  {
    front = true;
  }
  else
  ...
  // If either of the left IR sensors detect an object we set left to true
  /*if(!digitalRead(IR_LEFT_F) || !digitalRead(IR_LEFT_B))
  {
    left = true;
  }
  else
  ...
  // If either of the right IR sensors detect an object we set right to true
  if(!digitalRead(IR_RIGHT_F) || !digitalRead(IR_RIGHT_B))
  {
    right = true;
  }
  else
  ...
  // If either of the back IR sensors detect an object we set back to true
  if(!digitalRead(IR_BACK_L) || !digitalRead(IR_BACK_R))
  {
    back = true;
  }
  else
  ...
  // If only the front IR sensors detect an object move away from it
  if(front && !back)
  {
    backward();
  }
  // If only the back IR sensors detect an object move away from it
  else if(!front && back)
  {
    forward();
  }
  // If the drone is trapped from the front and back then it cannot move
  // forward or backward
  else if(front && back)
  {
    no_pitch();
  ...
  // If only the left IR sensors detect an object move away from it
  /*if(left && !right)
  {
    turn_right();
  }
  // If only the right IR sensors detect an object move away from it
  else if(!left && right)
  {
    turn_left();
  }
  // If the drone is trapped from the left and right then it cannot move
  // left or right
  else if(left && right)
  {
    no_roll();
